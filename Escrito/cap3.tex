...
Si CPPNs son evolucionadas para representar patrones de conectividad, el problema se dirige en encontrar la mejor interpretación de sus salidas para describir efectivamente la estructura que se intenta representar. EL patrón de dos dimensiones mostrado en la figura \ref{mapfenotipo} presenta un desafío: ¿ Cómo pueden patrones especiales describir la conectividad de una red ? Esta sección explica como patrones espaciales generados por CPPNs pueden mapear de manera natural patrones de conectividad de una red mientras que al mismo tiempo dan solución a problemas asignados a dicha red desde su propia dimensionalidad.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \label{fig:posio3}
    \end{subfigure}
    \caption{\textbf{Posicionando entradas y salidas. } Un robot (a) es descrito con ocho sensores lidar y ocho actuadores situados de forma radial en los mismos ángulos. En (b), las entradas (de etiqueta \textit{I}) y salidas (de etiqueta \textit{O}) están situadas en la misma diagonal en ocho direcciones distintas en el espacio. En (c), entradas y salidas son situadas horizontales y paralelamente una de la otra en dos filas. Ambos arreglos crean una relación geométrica entre cada entrada y su correspondiente salida. De esta forma es posible dar ventaja a la evolución desde el principio.}
    \label{fig:posio}
\end{figure}
